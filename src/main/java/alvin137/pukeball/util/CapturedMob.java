package alvin137.pukeball.util;
...
public class CapturedMob {
	...
	private CapturedMob(@Nonnull EntityLivingBase entity) {
		...
		if (entity instanceof EntitySkeleton) {
			variant = ((EntitySkeleton) entity).getSkeletonType();
		} else if (entity instanceof EntityZombie) {
			variant = ((EntityZombie) entity).getZombieType();
		} else {
			variant = null;
		}
	}
	...
	public static @Nullable CapturedMob create(@Nullable Entity entity) {
		if (!(entity instanceof EntityLivingBase) || entity.worldObj == null || entity.worldObj.isRemote
				|| entity instanceof EntityPlayer || isBlacklisted(entity)) {
			return null;
		}
	...
	public static boolean isBlacklisted(@Nonnull Entity entity) {
		...
		if (entityId == null || entityId.trim().isEmpty()
				|| (!Config.soulVesselCapturesBosses && !entity.isNonBoss())) {
			return true;
		}
	...
	public boolean spawn(@Nullable World world, @Nullable BlockPos pos, @Nullable EnumFacing side, boolean clone) {
		...
		if (theSide == EnumFacing.UP
				&& (blk instanceof BlockFence || blk instanceof BlockWall || blk instanceof BlockFenceGate)) {
			spawnY += 0.5;
		}
		...
		// riddenByEntity.setLocationAndAngles(spawnX, spawnY, spawnZ,
	...
	@SuppressWarnings("null")
	public @Nullable Entity getEntity(@Nullable World world, @Nullable BlockPos pos,
			@Nullable DifficultyInstance difficulty, boolean clone) {
		if (world == null) {
			return null;
		}
		...
		if (variant != null) {
			if (entity instanceof EntitySkeleton) {
				...
				skel.setSkeletonType((SkeletonType) variant);
				if (variant == SkeletonType.WITHER) {
					...
					if (Celeb.H31.isOn() && Math.random() < 0.25) {
						skel.setItemStackToSlot(EntityEquipmentSlot.HEAD,
								new ItemStack(Math.random() < 0.1 ? Blocks.LIT_PUMPKIN : Blocks.PUMPKIN));
						skel.setDropChance(EntityEquipmentSlot.HEAD, 0.0F);
					} else if (Celeb.C06.isOn() && Math.random() < 0.25) {
						skel.setItemStackToSlot(EntityEquipmentSlot.MAINHAND,
								new ItemStack(Math.random() < 0.25 ? Items.LEATHER_BOOTS : Items.STICK));
					}
				}
			} else if (entity instanceof EntityZombie) {
				boolean isChild = world.rand.nextFloat() < net.minecraftforge.common.ForgeModContainer.zombieBabyChance;
				((EntityZombie) entity).setZombieType((ZombieType) variant);
				if (((ZombieType) variant).isVillager()) {
					do {
						net.minecraftforge.fml.common.registry.VillagerRegistry
								.setRandomProfession((EntityZombie) entity, world.rand);
					} while (((EntityZombie) entity).getVillagerTypeForge() == null
							&& !((EntityZombie) entity).getZombieType().isVillager());
				}
				if (isChild) {
					((EntityZombie) entity).setChild(true);
					if (world.rand.nextFloat() < 0.05D) {
						EntityChicken entitychicken1 = new EntityChicken(world);
						entitychicken1.setLocationAndAngles(entity.posX, entity.posY, entity.posZ, entity.rotationYaw,
								0.0F);
	...
	public @Nonnull String getDisplayName() {
		...
		if (variant != null && SKELETON_ENTITY_NAME.equals(entityId)) {
			...
			String typeName = variant == SkeletonType.NORMAL ? entityId
					: variant == SkeletonType.WITHER ? "WitherSkeleton" : "Stray";
			...
			if (((ZombieType) NullHelper.notnullJ(variant, "private final field changed its value")).func_190154_b()) {
				...
				baseName = ((ZombieType) NullHelper.notnullJ(variant, "private final field changed its value"))
						.func_190145_d().getUnformattedText();
			}
		} else {
	...
	public boolean isSameType(Entity entity) {
		return entity != null && EntityList.getEntityString(entity) != null
				&& EntityList.getEntityString(entity).equals(getEntityName())
				&& (!(entity instanceof EntitySkeleton)
						|| isSameSkeleton(((EntitySkeleton) entity).getZombieType(), variant))
				&& (!(entity instanceof EntityZombie)
						|| isSameZombie(((EntityZombie) entity).getZombieType(), variant));
	}

	@Override
	public String toString() {
		return "CapturedMob [" + "entityId=" + entityId + ", "
				+ (customName != null ? "customName=" + customName + ", " : "") + "variant=" + variant + ", "
				+ (entityNbt != null ? "entityNbt=" + entityNbt + ", " : "") + "getDisplayName()=" + getDisplayName()
				+ ", getHealth()=" + getHealth() + ", getMaxHealth()=" + getMaxHealth() + ", "
				+ (getColor() != null ? "getColor()=" + getColor() + ", " : "")
				+ (getFluidName() != null ? "getFluidName()=" + getFluidName() : "") + "]";
	}

	// we treat normal zombies and all villager zombies as the same kind of
	// zombie for the pressure plate and the obelisks
	...
	/*
	 * Note: The Ender Dragon cannot be spawned as expected. All of its logic
	 * (moving, fighting, being hit, ...) is a special manager class, which is
	 * very hardcoded to the specifics of the vanilla dragon fight.
	 */ /*S
		...
		for (String mobName : mobs) {
			...
			if (soul != null && !"EnderDragon".equals(mobName)) {
				if (SKELETON_ENTITY_NAME.equals(mobName)) {
					...
					result.add(create(mobName, ZombieType.VILLAGER_BUTCHER)); // Forge
																				// will
																				// randomize
																				// these
